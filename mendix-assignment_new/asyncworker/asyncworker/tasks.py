...
def merge_s3_files(result):
    ...
    # check border dates
    if datetime.strptime(start_date, "%Y-%m-%d").date() < datetime.strptime(file_names[0][:10], "%Y-%m-%d").date():
        logger.exception('No files satisfying the request dates')

        return None

    if datetime.strptime(end_date, "%Y-%m-%d").date() > datetime.strptime(file_names[-1][:10], "%Y-%m-%d").date():
        logger.exception('No files satisfying the request dates')

        return None

    # find the first and last files in our selection
    start_file_index = [index for index, file in enumerate(file_names) if start_date in file][0]
    end_file_index = [index for index, file in enumerate(file_names) if end_date in file][-1]

    # get list of right files
    result_files_list = [file for file in list(file_names)[start_file_index:end_file_index + 1]]
    ...
    # put object
    s3_client.put_object(Bucket='results', Key=f"{result_filename}.log", Body=outputbody)
