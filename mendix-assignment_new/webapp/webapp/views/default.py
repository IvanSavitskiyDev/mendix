...
from celery.states import EXCEPTION_STATES, UNREADY_STATES
...
async def initiate(request):
    ...
    if task_result is None:
        return web.HTTPBadRequest(
            reason="Validation error", text="No files satisfying the request dates"
        )
...
async def download(request):
    ...
    if task_state in UNREADY_STATES:
        ...
        return web.json_response(
            {"result": f"Merge not yet complete, current status {task_state}"}
        )

    elif task_state in EXCEPTION_STATES:
        logger.exception(
            f"An error occurred while merging, current status {task_state}"
        )
        return web.json_response(
            {"result": f"An error occurred while merging, current status {task_state}"}
        )
